...
public class BoardsAdapter extends BaseAdapter {
    Button noColor, blueColor, pinkColor, skyBlueColor , greenColor, orangeColor, purpleColor, yellowColor,redColor,lightGreenColor,seaGreenColor,blackColor ;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(projectsList.get(position).getIsPicture().equals("0")) {
            ...
            Picasso.with(activity)
                    .load(End_Points.IMAGES_BASE_URL+"boards_background/"+projectsList.get(position).getBackGroundPicture().trim())
    ...
    private void startDialogForProfile(String boardId,String projectId,int position) {
                        ...
                        if (ActivityCompat.checkSelfPermission(activity, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                            ...
                            ((BoardsActivity)activity).uploadImage(boardId,projectId,position);
                           // upload();

                        }

    }
    private void upload(String boardId,String projectId,int position) {
        intent = new Intent(Intent.ACTION_PICK,
                MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
        intent.putExtra("bid",boardId);
        intent.putExtra("pos",position);
        activity.startActivityForResult(intent, IMG_RESULT);
    ...
    private void changeBoardBgColor(final String bgColor, final String boardId, final String projectId, final int position) {
